...
local function search(data)
  ...
  local params = {
    word = query,
    order = orderFilter:valueOfOrFirst(data[FID_ORDER]),
    mode = modeFilter:valueOfOrFirst(data[FID_MODE]),
    p = page,
    csw = 0,
  ...
  local selectedLang = languageFilter:valueOfOrFirst(data[FID_LANGUAGE])
...
return {
  ...
  listings = {
    ...
    Listing("Popular original novels", false, function (data)
      local genre = genreFilter:valueOfOrFirst(data[FID_GENRE])
      local mode = ((genre == "all" or genre == "for_kids") and "safe")
                or ((genre == "male" or genre == "female") and "r18")
                or modeFilter_popular:valueOfOrFirst(data[FID_MODE])
    ...
    Listing("Followed users 🔒", true, function (data)
      ...
      return parseListing(qs({
        p = data[PAGE],
        mode = modeFilter_followed:valueOfOrFirst(data[FID_MODE]),
        lang = "en"
